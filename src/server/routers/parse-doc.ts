...
import { checkFormSchema } from "@/lib/schemas/check-form";
...
export const parseDocProcedure = publicProcedure
  .input(checkFormSchema)
  .mutation(async ({ input }) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new TRPCError({
          code: "BAD_GATEWAY",
          message: `Google Docs returned ${res.status}. Make sure the document is shared as "Anyone with the link".`,
        });
      }
      html = await res.text();
    } catch (err) {
      ...
      throw new TRPCError({
        code: "BAD_GATEWAY",
        message: `Failed to fetch the Google Doc: ${
    ...
    for (const el of placeholderEls) {
      ...
      if (!driveId) {
        driveErrors.push(
          `${linkText}: could not extract Drive id from ${href}`,
        );
    ...
    const heading =
      $("body").find("h1, h2, h3").first().text().trim() || headTitle || null;
